...
(function () {
    ...
    function computeTelemetry() {
        ...
        if (simMode) {
            ...
            if (userSelectedDay) {
                ...
                activeDay = simDay || 1;
                const startDate = parseMetaDate(conferenceMeta.startDate || (conferenceMeta.days && conferenceMeta.days[0] && conferenceMeta.days[0].date) || '24/8');
    ...
    function getMealImage(act) {
        ...
        // الفطار حسب برنامج المؤتمر الجديد — اليوم الأول (24/8) ليس به فطار
        ...
        if (displayAct) {
            ...
            <div class="hero-stage-content">
                ...
                <div class="d-flex align-items-center justify-content-between" style="font-size:0.9rem; font-weight:900;">
                    ...
                    <span style="color:var(--retro-amber);"><i class="bi bi-hourglass-split me-1"></i>${isLive ? 'متبقي على الانتهاء: ' + remain + ' دقيقة' : (isPreConferenceDate ? 'تبدأ يوم ' + (displayAct.day === 1 ? '24 أغسطس' : (displayAct.day === 2 ? '25 أغسطس' : '26 أغسطس')) + ' الساعة ' + formatTime12(displayAct.time) : (remain > 0 ? 'تبدأ بعد: ' + remain + ' دقيقة' : 'موعد الفعالية: ' + formatTime12(displayAct.time)))}</span>
                </div>
